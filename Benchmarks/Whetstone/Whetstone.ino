...
/*
    COMMON T,T_1,T_2,E1(4),J,K,L
*/
float T, T_1, T_2, E1[5];
int J,K,L;
...
*/
    ...
    for (I = 1; I <= N7; I++) {
        X = T * DATAN(T_2*DSIN(X)*DCOS(X)/(DCOS(X+Y)+DCOS(X-Y)-1.0));
        Y = T * DATAN(T_2*DSIN(Y)*DCOS(Y)/(DCOS(X+Y)+DCOS(X-Y)-1.0));
    }

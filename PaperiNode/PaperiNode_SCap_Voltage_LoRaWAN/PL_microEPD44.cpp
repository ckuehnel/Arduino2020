...
void PL_microEPD::update(int updateMode) {
    ...
    switch (updateMode) {
        ...
        case 3:
            ...
            writeRegister(EPD_DISPLAYENGINE, 0x07, -1, -1, -1);   //Only changing pixels updated (delta mode)
...
void PL_microEPD::saveFBToFlash(int address) {
  ...
  for (int i=0; i<15; i++) {
      ...
      digitalWrite(cs, HIGH);
      waitForBusyInactive();
      flash_write(address+i*256, buffer, sizeof(buffer));
  }
}

void PL_microEPD::saveImgToFlash(int address) {
  ...
  for (int i=0; i<18; i++) {
      digitalWrite(cs, LOW);
      for (int j=0; j < sizeof(buffer); j++) {     //144 bytes
          buffer[j] = pgm_read_byte_near(pic + c);
          c++;
      }
      digitalWrite(cs, HIGH);
      waitForBusyInactive();
...
void PL_microEPD::loadFromFlash(int address, bool toPreviousBuffer) {
  writeRegister(EPD_WRITEPXRECTSET, 0, 69, 0, 147);
  if (toPreviousBuffer)
    writeRegister(EPD_DATENTRYMODE, 0x37, -1, -1, -1);        //Previous buffer @UC8156
  writeRegister(EPD_PIXELACESSPOS, 0, 0, -1, -1);       
  
  for (int i=0; i<18; i++) {
      memset(buffer, 0xFF, sizeof(buffer));
      flash_read(address+i*256, buffer, 144);
      digitalWrite(cs, LOW);
      SPI.transfer(0x10);

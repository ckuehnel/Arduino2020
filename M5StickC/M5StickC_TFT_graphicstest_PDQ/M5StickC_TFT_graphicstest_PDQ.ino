/*
 * File: M5StickC_TFT_graphictest_PDQ.ino
 * 
 * Graphic benchmark for M5StickC
 * 
...
uint32_t testImage()
{
  //tft.fillScreen(TFT_BLACK);
	uint32_t start = micros_start();
  M5.Lcd.drawBitmap(0, 0, image_width, image_height, gray_80x160_bits);
	uint32_t t = micros() - start;
	delay(3 * 1000L);
	return t;
}
...
uint32_t testRose()
{
  //tft.fillScreen(TFT_BLACK);
  uint32_t start = micros_start();
  M5.Lcd.drawBitmap(0, 0, image_width, image_height, Rose_80x160_bits);
  uint32_t t = micros() - start;
  delay(3 * 1000L);
  return t;
}
